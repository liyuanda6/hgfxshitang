...
/* ---------------------------- 总报表（各班餐费合计）工作表 ---------------------------- */
function summarySheetXml(sh) {
  const colWidths = [6, 20, 16, 30];
  let colsXml = '<cols>';
  colWidths.forEach((w, i) => {
    colsXml += '<col min="' + (i + 1) + '" max="' + (i + 1) + '" width="' + w + '" customWidth="1"/>';
  });
  ...
  for (const row of sh.rows) {
    const cells = [
      '<c r="A' + r + '" s="3"><v>' + row.seq + '</v></c>',
      '<c r="B' + r + '" s="4" t="inlineStr"><is><t xml:space="preserve">' + xmlEscape(row.name) + '</t></is></c>',
      '<c r="C' + r + '" s="5"><v>' + round2(row.amount) + '</v></c>',
      '<c r="D' + r + '" s="4" t="inlineStr"><is><t xml:space="preserve">' + xmlEscape(row.remark || '') + '</t></is></c>'
    ].join('');
    rows.push('<row r="' + r + '" ht="20">' + cells + '</row>');
    r++;
  }
  ...
  // 合计行
  const tr = r;
  const sum = sh.rows.reduce((a, b) => a + round2(b.amount), 0);
  let totalCells = '<c r="A' + tr + '" s="6" t="inlineStr"><is><t xml:space="preserve">合计</t></is></c>';
  totalCells += '<c r="B' + tr + '" s="4"/>';
  totalCells += '<c r="C' + tr + '" s="7"><f>SUM(C3:C' + lastData + ')</f><v>' + round2(sum) + '</v></c>';
  totalCells += '<c r="D' + tr + '" s="4"/>';
  rows.push('<row r="' + tr + '" ht="20">' + totalCells + '</row>');

  // 底部签名行：附小后勤负责人 / 领导审核 + 日期
  const sig1 = tr + 2;
  const sig2 = sig1 + 1;
  merges.push('A' + sig1 + ':B' + sig1);
  merges.push('A' + sig2 + ':B' + sig2);
  merges.push('C' + sig2 + ':D' + sig2);
  rows.push('<row r="' + sig1 + '" ht="20"><c r="A' + sig1 + '" s="9" t="inlineStr"><is><t xml:space="preserve">附小后勤负责人：________________</t></is></c></row>');
  rows.push('<row r="' + sig2 + '" ht="20"><c r="A' + sig2 + '" s="9" t="inlineStr"><is><t xml:space="preserve">领导审核：________________</t></is></c>' +
    '<c r="C' + sig2 + '" s="10" t="inlineStr"><is><t xml:space="preserve">日期：        年      月      日</t></is></c></row>');

  const dimension = '<dimension ref="A1:D' + sig2 + '"/>';
  const sheetViews = '<sheetViews><sheetView workbookViewId="0"><pane ySplit="2" topLeftCell="A3" activePane="bottomLeft" state="frozen"/></sheetView></sheetViews>';
  const sheetFormatPr = '<sheetFormatPr defaultRowHeight="18"/>';
  const sheetData = '<sheetData>' + rows.join('') + '</sheetData>';
  const mergeXml = '<mergeCells count="' + merges.length + '">' +
...
function renderMealWorkbook(sheets) {
  const safeSheets = sheets.map((s, idx) => ({
    ...
    xml: s.type === 'summary' ? summarySheetXml(s) : sheetXml(s)
  }));
  // 同名处理
  const seen = {};
